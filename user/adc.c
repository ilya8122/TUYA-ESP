...
extern xQueueHandle xQueueCreateMutex( unsigned char ucQueueType ) PRIVILEGED_FUNCTION;//TUYA SDK
#define xSemaphoreCreateMutex() xQueueCreateMutex( queueQUEUE_TYPE_MUTEX )

#define queueSEND_TO_BACK       ( ( portBASE_TYPE ) 0 )
#define semGIVE_BLOCK_TIME                  ( ( portTickType ) 0U )
//extern  xSemaphoreGive( xSemaphore );
extern signed portBASE_TYPE xQueueGenericSend( xQueueHandle xQueue, const void * const pvItemToQueue, portTickType xTicksToWait, portBASE_TYPE xCopyPosition ) PRIVILEGED_FUNCTION;
#define xSemaphoreGive( xSemaphore )        xQueueGenericSend( ( xQueueHandle ) ( xSemaphore ), NULL, semGIVE_BLOCK_TIME, queueSEND_TO_BACK )


//extern  xSemaphoreTake( xSemaphore, xBlockTime );
extern signed portBASE_TYPE xQueueGenericReceive( xQueueHandle xQueue, const void * const pvBuffer, portTickType xTicksToWait, portBASE_TYPE xJustPeek ) PRIVILEGED_FUNCTION;//TUYA SDK
...
*/
 //   adc_handle = heap_caps_malloc(sizeof(adc_handle_t), MALLOC_CAP_8BIT);//        In SDK, ``malloc(s)`` is equivalent to ``heap_caps_malloc(s, MALLOC_CAP_32BIT)``.
